...
{
    public partial class AlterCustomerProcedureAddLastNameFilter : Migration
    {
...
BEGIN
    ...
    SET @filter = LTRIM(RTRIM(@filter));
            ...
            (
                ...
                WHERE (@filter IS NULL OR FirstName LIKE '%' + @filter + '%' OR LastName LIKE '%' + @filter + '%')
	...
	CTE_TotalRows AS
    (
     select count(Id) as MaxRows from Customers WHERE (@filter IS NULL OR FirstName LIKE @filter +'%')
	)
